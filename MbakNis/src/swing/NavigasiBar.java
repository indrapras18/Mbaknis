...
public class NavigasiBar extends javax.swing.JPanel{
    ...
    public void addItem(Icon icon) {
        item.add(new ModelNavigasiBar(icon, item.size(), null));
        repaint();
    }

    private int getSelectedIndex(Point mouse) {
        for (ModelNavigasiBar i : item) {
    ...
    public void setBounds(int i, int i1, int i2, int i3) {
        ...
        if (selectedIndex >= 0) {
            if (!animator.isRunning()) {
                ModelNavigasiBar d = item.get(selectedIndex);
    ...
    private void runBeforeEvent() {
        for (EventNavigasiBar event : events) {
    ...
    public void addEvent(EventNavigasiBar event) {
    ...
    public void setnavigasiBackgroundColor(NavigasiBackgroundColor nb){
        addEvent(new EventNavigasiBar() {
            @Override
